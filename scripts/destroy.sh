...
#
# Total Destruction Script for AWS EKS Infrastructure
...
# Confirmation
echo -e "${RED}WARNING: This will permanently delete ALL resources for ${DEPLOYMENT_ID}-eks${NC}"
...
cleanup_remaining() {
    ...
    if [[ -z "$s3_remaining" && -z "$secrets_remaining" ]]; then
